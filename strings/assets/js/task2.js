...
let stringsTask2 = (function () {
    const task = "Task 2";
    const problem = ` Да се напише функция, която приема масив от обекти
                    (ползвайте масива от предната задача) като първи параметър и
                    стринг като втори параметър. Функцията трябва да връща масив
                    от тези обекти, които съдържат поне един таг, който е равен
                    на стринга от втория параметър('Lorem').Ако не намери
                    съвпадение, функцията да връща null. where(arr, 'Ipsum');`;
